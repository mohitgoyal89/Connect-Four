...
# Python 2.3 compatibility with sets
...
class InvalidMoveException(Exception):
    ...
    def __unicode__(self):
        return "InvalidMoveException: Can't add to column %s on board\n%s" % (
            unicode(self._column), unicode(self._board))
...
class ConnectFourBoard(object):
    ...
    def __init__(self, k=4, board_array=None, board_already_won=None, modified_column=None, current_player=1,
                 previous_move=-1):
        ...
        """
        # K holds the streak size for win situation for connect k game
        self.K = k
    ...
    def do_move(self, column):
        ...
        # Added logic for Connect k
        return ConnectFourBoard(self.K, new_board, board_already_won=self.is_win(), modified_column=column,
    ...
    def _is_win_from_cell(self, row, col):
        ...
        return (self._max_length_from_cell(row, col) >= self.K)  # replaced 4 with k
    ...
    def getWinningRowCount(self, row, col, playerId):
        """
        It computes the winning row count for a player for given cell position horizontally, vertically and  diagonally
        Winning position is a array of cell size = k such that it has atleast one token of player and no token of
        opponent. FOr each we check that is there any winning position or not. More number of winning positions means
        chances of winning.
        """
        k = self.K
        count = 0
        ...
        i = row
        j = col
        # Check right side win
        while j < col + k and j < self.board_width and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            j += 1
        if count == k:
            win += 1
        ...
        i = row
        j = col
        while j > col - k and j >= 0 and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            j -= 1
        if count == k:
            win += 1
        # check vertical up side win
        count = 0
        i = row
        j = col
        while i > row - k and i >= 0 and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            i -= 1
        if count == k:
            win += 1

        # check vertical down side win
        count = 0
        i = row
        j = col
        while i < row + k and i < self.board_height and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            i += 1
        if count == k:
            win += 1
        # check diagonally up positive
        count = 0
        i = row
        j = col
        while i > row - k and i >= 0 and j < col + k and j < self.board_width \
                and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            i -= 1
            j += 1
        if count == k:
            win += 1
        # check diagonally down positive side
        count = 0
        i = row
        j = col
        while i < row + k and i < self.board_height and j < col + k and j < self.board_width \
                and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            i += 1
            j += 1
        if count == k:
            win += 1
        # check diagonally down negative  side
        count = 0
        i = row
        j = col
        while i < row + k and i < self.board_height and j > col - k and j >= 0 \
                and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            i += 1
            j -= 1
        if count == k:
            win += 1
        # check diagonally up negative  side
        count = 0
        i = row
        j = col
        while i > row - k and i >= 0 and j > col - k and j >= 0 \
                and (playerId == self.get_cell(i, j) or self.get_cell(i, j) == 0):
            count += 1
            i -= 1
            j -= 1
    ...
    def _chain_sets_from_cell(self, row, col):
        ...
        return [tuple(x) for x in [
            reverse(self._contig_vector_cells(row, col, (1, 1))) + [(row, col)] + self._contig_vector_cells(row, col,
                                                                                                            (-1, -1)),
            reverse(self._contig_vector_cells(row, col, (1, 0))) + [(row, col)] + self._contig_vector_cells(row, col,
                                                                                                            (-1, 0)),
            reverse(self._contig_vector_cells(row, col, (0, 1))) + [(row, col)] + self._contig_vector_cells(row, col,
                                                                                                            (0, -1)),
            reverse(self._contig_vector_cells(row, col, (-1, 1))) + [(row, col)] + self._contig_vector_cells(row, col,
                                                                                                             (1, -1))
        ]]
    ...
    def clone(self):
        ...
        # Added logic for connect K
        return ConnectFourBoard(self.K, self._board_array, board_already_won=self._is_win,
    ...
    def __unicode__(self):
        ...
        retVal += [unicode(i) + ' ' + u' '.join([self.board_symbol_mapping[x] for x in row]) for i, row in
                   enumerate(self._board_array)]
        return u'\n' + u'\n'.join(retVal) + u'\n'

    def __str__(self):
        """ Return a string representation of this board """
        retVal = ["  " + ' '.join([str(x) for x in range(self.board_width)])]
        retVal += [str(i) + ' ' + ' '.join([self.board_symbol_mapping_ascii[x] for x in row]) for i, row in
                   enumerate(self._board_array)]
        return '\n' + '\n'.join(retVal) + '\n'
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def run_game(self, verbose=True):
        ...
        while not win_for_player and not self._board.is_tie():
            ...
            for callback, id, symbol in (player1, player2):
                ...
                # After 20 moves, find the max streak of each player
                # Whoever has the largest streak vertically, horizontally or diagonally wins
                # if both have same streak then its a tie

                # Uncomment the code below to test it

                if moveCount > 20:
                    p1_streak = self.find_max_streak(player1[1])
                    p2_streak = self.find_max_streak( player2[1])
                    if p1_streak > p2_streak:
                        print "Player 1 won with streak =%d" % (p1_streak)
                    elif p1_streak < p2_streak:
                        print "Player 2 won with streak =%d" % (p2_streak)
                    else:
                        print "Player 1 and Player 2 both have a tie with streak = %d" % (p1_streak)
                    return
        ...
        if win_for_player != 0 and self._board.is_tie():
            ...
            if self.bpNodesExpanded != 0:
                print "basic player Nodes expanded = " + str(self.bpNodesExpanded)
            if self.npNodesExpanded != 0:
                print "new player Nodes expanded = " + str(self.npNodesExpanded)
            if self.abNodesExpanded != 0:
                print "alpha beta Nodes expanded = " + str(self.abNodesExpanded)

            self.totNodesExpanded = self.bpNodesExpanded + self.npNodesExpanded + self.abNodesExpanded
            print "Total Nodes expanded = " + str(self.totNodesExpanded)
            return 0
        else:
    ...
    def _do_gameend(self, winner):
        ...
        if self._board.board_symbol_mapping[winner] != ' ':
    ...
    def find_max_streak(self, player_id):
        """ It finds the maximum streak  vertically, horizontally or diagonally for a player in the board state
        """
        return self._board.longest_chain(player_id)
